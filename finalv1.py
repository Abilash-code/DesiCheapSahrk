    ...
    if epic_url == "" :
        ...
        try :
            with sync_playwright() as p :
                ...
                page.goto(epic_url)
                outer_span = page.locator("span.eds_1ypbntd0.eds_1ypbntdb.eds_1ypbntdk").all()
                for thing in outer_span :
                    free_price = thing.locator("strong").all()
                    for price in free_price :
                        value = price.locator("span")
                        if value.count() != 0 :
                            if value.text_content() == "Free" :
                                free_on_epic = True
